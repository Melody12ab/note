...
<head>
    <meta charset="UTF-8">
    <title>lennovo</title>
    <script src="http://api.map.baidu.com/api?v=2.0&ak=Djzu4HmZm90n9Ap2xCXhWFcV"></script>
...
function BMap_init(){
    var citySlected=document.getElementById("city");
    var index=citySlected.selectedIndex;
    var name=citySlected.options[index].text;
    initMap(name);
}
...
function InitCitySelect(selProvince,selCity)
{
    ...
    g_selCity=selCity;
    selCity.onchange=Function("BMap_init();");
...
function product_onchange(){
    ...
    //var url="http://support.lenovo.com.cn/lenovo/wsi/station/servicestation/Api/QueryMap.ashx?area="+encodeURI(name+'市')+"&type="+encodeURI(value);

    $.ajax({
        url:"http://localhost/test/mapD.php",
        dataType:"json",
        type:"GET",
        data:{area:name+'市',type:value},
...
function InitCitySelect(selProvince,selCity,selProduct)
{
    g_selProvince=selProvince;
    g_selCity=selCity;
    selProduct.onchange=Function("product_onchange();");
    selCity.onchange=Function("BMap_init();");
    selProvince.onchange=Function("Province_onchange();");
    FillProvinces(selProvince);
    Province_onchange();
...
<select id="product" size="1" name="product">
    <option value="-1" selected="selected">请选择</option>
    <option value="笔记本">联想笔记本</option>
    <option value="台式">联想台式机</option>
    <option value="ThinkPad">ThinkPad笔记本</option>
    <option value="NewThinkCentre">ThinkCentre台式机</option>
    <option value="服务器">ThinkServer服务器</option>
    <option value="ThinkStation">ThinkStation工作站</option>
    <option value="ThinkVision">ThinkVision显示器</option>
    <option value="ThinkCentre">Old ThinkCentre台式机</option>
    <option value="1">SmartTV智能电视</option>
    <option value="-5">联想手机/Tablet</option>
    <option value="选件">选件</option>
    <option value="打印机、多功能一体机">打印机、多功能一体机</option>
    <option value="投影机">投影机</option>
    <option value="光墨打印机">光墨打印机</option>
    <option value="4">昭阳加固型笔记本</option>
</select>

<script>
InitCitySelect(document.profile.province,document.profile.city,document.profile.product);
//document.getElementById("product").onchange=Function("product_onchange();");
        ...
        <script type="text/javascript">
            ...
            for (var i = 0; i < 25; i ++) {
                ...
                var label = new BMap.Label(i,{offset:new BMap.Size(2)});
                label.setStyle({color:"white",fontSize:"12px",backgroundColor:"#ED2D2D",border:"0",borderRadius:"10px"});
